         ...
         <div class="min-h-screen flex flex-col">
             ...
             <header class="bg-secondary text-white p-4">
                 <div class="container mx-auto flex justify-between items-center">
                     <h1 class="text-xl font-bold">{{__('web.Add New Assurance')}}</h1>
             ...
             <main class="container mx-auto mt-10">
                 <div class="max-w-lg mx-auto bg-beige p-6 rounded-lg shadow-lg">
                     <h2 class="text-2xl text-primary font-bold mb-6 text-center">{{__('web.Add New Assurance')}}</h2>
                     <form action="{{ route('assurances.store') }}" method="POST">
                         ...
                         <div class="mb-4">
                             ...
                             <select id="request_number" name="request_number"
                                     class="w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-secondary" onchange="updateItemNameAndCustomerName()">
                                 <option value="">{{__('web.Select Request Number')}}</option>
                                 @foreach ($requests as $request)
                                     <option value="{{ $request->id }}"
                                             data-item-name="{{ $request->item->name }}"
                                             data-customer-name="{{ $request->user->first_name }} {{ $request->user->last_name }}"
                                             data-item-id="{{ $request->item->id }}"> <!-- Add item id here -->
                         ...
                         <input type="hidden" id="rental_request_id" name="rental_request_id">
 
                         <!-- Amount -->
                         <div class="mb-4">
                             <label for="amount" class="block text-secondary font-medium mb-2">{{__('web.Assurance Amount')}}</label>
                             <input type="number" id="amount" name="amount" value="{{$request->item->price_assurance}}"
                         ...
                         <div class="mb-4">
                             ...
                             <textarea id="en_description" name="en_description" rows="4"
                                       class="w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-secondary"
                                       placeholder="Enter English Description" required></textarea>
                         </div>
                         ...
                         <div class="mb-4">
                             ...
                             <textarea id="ar_description" name="ar_description" rows="4"
                                       class="w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-secondary"
                                       placeholder="أدخل الوصف بالعربية" required></textarea>
                         </div>
 
                         {{-- <!-- Submit Button -->
                         <div class="flex justify-between">
                             <button type="submit" class="bg-primary text-white py-2 px-6 rounded hover:bg-secondary">Add Assurance</button>
                             <a href="/assurances"
                                 class="bg-sky_blue text-black py-2 px-4 rounded hover:bg-gray-200">Cancel</a>
                         </div>
                         --}}
                         <!-- Submit Button -->
                         <div class="flex justify-between">
                             <button type="submit" class="bg-primary text-white py-2 px-6 rounded hover:bg-secondary">{{__('web.Add Assurance')}}</button>
