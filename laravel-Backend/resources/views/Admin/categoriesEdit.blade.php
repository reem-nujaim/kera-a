    ...
    <div class="bg-white text-gray-800 pb-6">
        ...
        <header class="bg-secondary text-white p-4">
          <div class="container mx-auto flex justify-between items-center">
            <h1 class="text-xl font-bold">{{__('web.Edit Category')}}</h1>
        ...
        <main class="container mx-auto mt-10">
            <div class="max-w-lg mx-auto bg-beige p-6 rounded-lg shadow-lg">
                <h2 class="text-2xl font-bold text-primary mb-6 text-center">{{__('web.Edit Category')}}</h2>
                ...
                <form action="{{ route('categories.update', $category->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="category-name" name="en_name"
                            ...
                            placeholder="Enter category name" value="{{ $category->en_name }}" required />
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="category-name" name="ar_name"
                            class="w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-secondary"
                            placeholder="Enter category name" value="{{ $category->ar_name }}" required />
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="category-description" name="en_descrieption" rows="4"
                            class="w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-secondary"
                            placeholder="Enter category description" required>{{ $category->en_descrieption }}</textarea>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="category-description" name="ar_description" rows="4"
                            class="w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-secondary"
                            placeholder="Enter category description" required>{{ $category->ar_description }}</textarea>
                    </div>

                    <!-- Parent Category (Dropdown for selecting parent category) -->
                   
                    <div class="mb-4">
                      <label for="parent-category" class="block text-sm font-medium text-secondary mb-2">{{__('web.Main Category')}}</label>
                      <select name="parent_id" id="parent-category" class="w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-secondary">
                          <option value="">{{__('web.Select As Main Category')}}</option>
                          @foreach ($categories as $parent)
                              @if ($parent->id != $category->id) <!-- استبعاد الفئة الحالية -->
                                  <option value="{{ $parent->id }}" @if ($category->parent_id == $parent->id) selected @endif>
                                      {{ $parent->en_name }} - {{ $parent->ar_name }}
                                  </option>
                              @endif
                          @endforeach
                      </select>
